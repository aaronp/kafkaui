...
class JsonWebToken(val header: JwtHeader, val claims: Claims, val signature: String) {
  ...
  private def validateSignature(secret: Array[Byte]): Boolean = {
    ...
    util.Arrays.equals(encoded, signature.getBytes(StandardCharsets.UTF_8))
  }
...
object JsonWebToken {
  ...
  def hmac256SignPayload(payload: String, secret: SecretKeySpec): String = Hmac256.encodeToString(secret, payload)
  ...
  private def fromBase64(base64Str: String): String = {
    new String(Base64.getUrlDecoder.decode(base64Str), StandardCharsets.UTF_8)
  }

  def forToken(jwt: String, secret: SecretKeySpec): Either[JwtError, JsonWebToken] = {
    parseToken(jwt).flatMap {
  ...
  def parseToken(jwt: String): Either[JwtError, JsonWebToken] = {
    jwt match {
      case JwtR(headerB64, claimsB64, signature) =>
        val headerE= JwtHeader.fromJson(fromBase64(headerB64))

        val headerAndClaims = headerE.flatMap { header =>
          val claimsE = Claims.fromJson(fromBase64(claimsB64))
          claimsE.map { c =>
